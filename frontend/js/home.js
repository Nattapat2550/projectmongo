...
async function initCarousel() {
  ...
  // ไม่มีสไลด์ -> แสดง placeholder
  if (!Array.isArray(items) || items.length === 0) {
    track.innerHTML = `
      <div class="carousel-item active">
        <img class="slide-img" src="/images/user.png" alt="Placeholder">
      </div>`;
  ...
  const renderDesc = (i) => {
    ...
    const des = (it.description || '').toString();
    descEl.innerHTML = `
      ${sub ? `<div class="cap-sub">${escapeHtml(sub)}</div>` : ''}
      ${des ? `<div class="cap-desc">${escapeHtml(des)}</div>` : ''}
    `;
  };
  ...
  carousel.addEventListener('touchend', (e) => {
    ...
    const th = 40;
    if (dx > th)  goTo(current - 1);
    if (dx < -th) goTo(current + 1);
    startX = null;
  }, { passive: true });
}

// Safe HTML escape (แก้ SyntaxError เดิม)
